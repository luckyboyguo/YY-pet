...
class UIManager {
    constructor(dataManager, soundManager, petManager) {
        ...
        this.parentAuthMode = 'admin';
        this.pendingScoreDelta = null;
        this.init();
    }
    ...
    renderPetGrid() {
        ...
        grid.innerHTML = '';
        const isSingleStudentMode = this.data.students.length === 1;
        document.body.classList.toggle('single-student-mode', isSingleStudentMode);
        grid.classList.toggle('single-student-grid', isSingleStudentMode);
        ...
        this.data.students.forEach((student, index) => {
            ...
            const stageKey = (bgClass || '').replace('stage-', '');
            ...
            if (isSingleStudentMode) {
                ...
                card.classList.add('single-student-card');
                card.innerHTML = `
                    <div class="single-mode-label">家庭专属宠物</div>
                    ...
                    <div class="stage-badge ${bgClass}">${petInfo ? petInfo.stageName : stage.name}</div>
                    <div class="single-progress">
                        <div class="single-progress-track">
                            <div class="single-progress-fill" style="width: ${progress}%"></div>
                        </div>
                        <div class="single-progress-text">距离下一阶段还需 ${needScore > 0 ? needScore : 0} 分</div>
                    </div>
                    <div class="single-actions-hint">点击卡片可加减分、宠物互动、兑换奖励</div>
                `;
            } else {
                card.innerHTML = `
                    <div class="pet-image-container ${bgClass}">
    ...
    updatePetCard(studentId) {
        ...
        const bgClass = this.petDisplay.getPetBgClass(student.score);
        const stage = this.data.getStage(student.score);
        card.dataset.stage = (bgClass || '').replace('stage-', '');
        ...
        if (badge) {
            badge.textContent = petInfo ? petInfo.stageName : stage.name;
            badge.className = `stage-badge ${bgClass}`;
        }

        const singleProgressFill = card.querySelector('.single-progress-fill');
        const singleProgressText = card.querySelector('.single-progress-text');
        if (singleProgressFill) {
            singleProgressFill.style.width = `${this.data.getProgress(student.score)}%`;
        }
        if (singleProgressText) {
            const needScore = this.data.getNeedScore(student.score);
            singleProgressText.textContent = `距离下一阶段还需 ${needScore > 0 ? needScore : 0} 分`;
        }
    }
    ...
    document.getElementById('btnAdoption').addEventListener('click', () => {
        ...
        document.getElementById('btnTeacher').addEventListener('click', () => {
            this.openParentAuthModal('admin');
            this.sound.playClick();
        });
        ...
        document.getElementById('btnCancelTeacher').addEventListener('click', () => {
            ...
            if (this.parentAuthMode === 'score') {
                this.pendingScoreDelta = null;
            }
        });

        document.getElementById('btnEnterAdmin').addEventListener('click', () => {
            this.checkPassword(document.getElementById('teacherPassword').value, this.parentAuthMode);
        });

        // 密码输入框支持回车键
        document.getElementById('teacherPassword').addEventListener('keyup', (e) => {
            if (e.key === 'Enter') {
                this.checkPassword(e.target.value, this.parentAuthMode);
            }
        });
    ...
    openParentAuthModal(mode = 'admin') {
        this.parentAuthMode = mode;
        const titleElement = document.getElementById('teacherModalTitle');
        const descElement = document.getElementById('teacherModalDesc');
        const actionButton = document.getElementById('btnEnterAdmin');
        const errorElement = document.getElementById('teacherError');
        const passwordInput = document.getElementById('teacherPassword');

        if (mode === 'score') {
            if (titleElement) titleElement.textContent = '🔒 家长验证';
            if (descElement) descElement.textContent = '仅家长可以加减分，请输入家长密码';
            if (actionButton) actionButton.textContent = '验证并继续';
        } else {
            if (titleElement) titleElement.textContent = '🔐 家长模式';
            if (descElement) descElement.textContent = '请输入家长密码进入家长端';
            if (actionButton) actionButton.textContent = '确认进入';
        }
    ...
    handleScoreChange(delta) {
        ...
        this.openParentAuthModal('score');
    }
    ...
    checkPassword(password, mode = 'admin') {
        ...
        if (password === this.data.config.teacherPassword) {
            ...
            if (mode === 'score') {
                const pendingDelta = this.pendingScoreDelta;
                this.pendingScoreDelta = null;

                if (typeof pendingDelta === 'number') {
                    this.applyScoreChange(pendingDelta);
                }

                alert('家长验证成功，已执行本次加减分');
            } else {
                // 跳转到管理页面
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('focus', () => {
        dataManager.loadConfig();
        dataManager.students = dataManager.loadStudents();
        uiManager.renderPetGrid();
        uiManager.updateClassName();
    });
